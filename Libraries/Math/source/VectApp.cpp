...
namespace GD
{
	...
	//void VectApp::InverseLerp(float& out, const Vect& a, const Vect& b, const Vect& val)
	//{
	//	//Fix this, this is returning a vector and I need a float
	//	//out = (val - a) / (b - a);
	//}
	void VectApp::PrintVect(const Vect& in)
	{
		GDWriter::write("- - x: %f,  y: %f,  z: %f,  w: %f\n", 
					   in.X(), in.Y(), in.Z(), in.W());
	}
}
